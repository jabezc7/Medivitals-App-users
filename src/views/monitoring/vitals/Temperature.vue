    ...
    <card>
        <template #heading>
            <div class="flex flex-row items-center">
                <span class="flex-grow">Temperature</span>
                <div class="flex sm:mt-0 sm:ml-4 gap-2 items-center">
                    ...
                    <a v-if="isTemperatureCont" href="Javascript:void(0)" @click="isTemperatureCont = !isTemperatureCont"><font-awesome-icon :icon="['fas', 'chevron-up']" class="text-xl pt-2 font-black text-gentian-blue" /></a>
                    <a v-else href="Javascript:void(0)" @click="isTemperatureCont = !isTemperatureCont"><font-awesome-icon :icon="['fas', 'chevron-down']" class="text-xl font-black  pt-2 text-gentian-blue" /></a>
                </div>
            </div>
        </template>
        <template #content>
            <div v-if="isTemperatureCont" class="grid grid-cols-1 lg:gap-6 sm:grid-cols-1 lg:grid-cols-3">
...
<script setup>
    ...
    const isTemperatureCont = ref(true)
    const temperature = ref()

    async function submit() {
        let value = temperature.value
        await data.insert('temperature', value, patient.details.id)
