    ...
    <card>
        <template #heading>
            <div class="flex flex-row items-center">
                <span class="flex-grow">Saturation Rate</span>
                <div class="flex sm:mt-0 sm:ml-4 gap-2 items-center">
                    ...
                    <a v-if="isSaturationCont" href="Javascript:void(0)" @click="isSaturationCont = !isSaturationCont"><font-awesome-icon :icon="['fas', 'chevron-up']" class="text-xl pt-2 font-black text-gentian-blue" /></a>
                    <a v-else href="Javascript:void(0)" @click="isSaturationCont = !isSaturationCont"><font-awesome-icon :icon="['fas', 'chevron-down']" class="text-xl font-black  pt-2 text-gentian-blue" /></a>
                </div>
            </div>
        </template>
        <template #content>
            <div v-if="isSaturationCont" class="grid grid-cols-1 lg:gap-6 sm:grid-cols-1 lg:grid-cols-3">
...
<script setup>
    ...
    const isSaturationCont = ref(true)

    async function submit() {
        let value = saturation.value
        await data.insert('oxygen_saturation', value, patient.details.id)

        createToast('Saturation rate successfully added.', {
