    ...
    <card>
        <template #heading>
            <div class="flex flex-row items-center">
                <span class="flex-grow">Respiratory Rate</span>
                <div class="flex sm:mt-0 sm:ml-4 gap-2 items-center">
                    ...
                    <a v-if="isRespiratoryCont" href="Javascript:void(0)" @click="isRespiratoryCont = !isRespiratoryCont"><font-awesome-icon :icon="['fas', 'chevron-up']" class="text-xl pt-2 font-black text-gentian-blue" /></a>
                    <a v-else href="Javascript:void(0)" @click="isRespiratoryCont = !isRespiratoryCont"><font-awesome-icon :icon="['fas', 'chevron-down']" class="text-xl font-black  pt-2 text-gentian-blue" /></a>
                </div>
            </div>
        </template>
        <template #content>
            <div v-if="isRespiratoryCont" class="grid grid-cols-1 lg:gap-6 sm:grid-cols-1 lg:grid-cols-3">
                    ...
                    <div class="divmin-height-2x px-4 py-10 bg-white  border-2 border-gray-200 rounded-lg overflow-hidden sm:p-6 text-center">
                        <div class="w-full gap-2 mb-5 px-4 grid grid-cols-3 sm:grid-cols-6">
                            <a v-for="(filter, x) in filters"
                                ...
                                @click="setFilter(filter, i, chart.title)"
                            >{{ filter.title }}</a>
                        </div>
...
<script setup>
    ...
    const isRespiratoryCont = ref(true)
    const bpm = ref()
