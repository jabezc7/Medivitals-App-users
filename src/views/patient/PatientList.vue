                    ...
                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        <table class="min-w-full divide-y divide-gray-300">
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                <tr v-for="(item, i) in patient.list.data" :key="i">
                                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-500 sm:pl-6">
                                        {{ item.full_name }}
                                    </td>
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                        {{ item.assignee.first }} {{ item.assignee.last }}
                                    </td>
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                    ...
                                    <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 text-gentian-blue">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 cursor-pointer" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" @click="goToPatient(item.id)">
...
<script setup>
    ...
    async function debounceSearch(event) {
        ...
        debounce.value = setTimeout(async() => {
            ...
            await getList(search.value, page.value)
        }, 600)
    }
    ...
    onMounted(async() => {
        await filter.setLimit(10)
        page.value = 1
        await getList(search.value, page.value)
    })

    async function getList(value, page) {
        await filter.setSearchFilter(value, 'datatable')
        await filter.setPage(page)
        await patient.getList(filter.params)
        total.value = patient.list.meta.total
        lastPage.value = patient.list.meta.last_page
    }
